...
<%-- Beginning of custom code - 2fa validation --%>
...
<%-- End of custom code - 2fa validation --%>
...
<%-- Beginning of custom code - 2fa validation --%>
<c:set var="ignoredQueryParams" value=",2fa,loginOp,loginNewPassword,totpcode,loginConfirmNewPassword,loginErrorCode,username,email,password,zrememberme,ztrusteddevice,zlastserver,client,login_csrf,"/>
<%-- End of custom code - 2fa validation --%>
<%-- get useragent --%>
...
<c:if test="${not empty authResult}">
        <c:choose>
            ...
            <c:otherwise>
                <%-- Beginning of custom code - 2fa validation --%>
            	<c:choose>
                    <c:when test="${!(empty param.username) && !(empty param.password) && !validTwoFa}">
                        <c:set var="errorCode" value="errorPassChange"/>
                        <c:set var="errorMessage" value="${twoFactorAuthError}"/>
                        <zm:logout/>
                    </c:when>
                    <c:otherwise>
                <%-- End of custom code - 2fa validation --%>
                    ...
                    <c:otherwise>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                ...
                                <%-- Beginning of custom code - 2fa validation --%>
				                <tr>
                                    <td><label for="2fa">2FA:</label></td>
                                    <td><input id="2fa" autocomplete="off" class="zLoginField" name="2fa" type="text" value="" size="40" maxlength="${domainInfo.webClientMaxInputBufferLength}"/></td>
				                </tr>
				                <%-- End of custom code - 2fa validation --%>
